...
def load_data(data_dir):
    ...
    for cat in range(0, NUM_CATEGORIES):
        ...
        dir = os.path.join(data_dir, str(cat))
        for img_path in os.listdir(dir):
            # get the path of each image inside the directory
            path = os.path.join(data_dir, str(cat), img_path)
            # read the image
            image = cv2.imread(path)
            image = cv2.resize(image, (IMG_WIDTH,IMG_HEIGHT))
            image = image.astype(np.float32) / 255  # Normalization
            image = image.astype(np.float32,copy = False)  # shape of im = (30,30,3)
            images.append(image)
            labels.append(cat)
    print(len(images))
